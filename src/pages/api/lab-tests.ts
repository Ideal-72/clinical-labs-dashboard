...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!isSupabaseConfigured()) {
    switch (method) {
      case 'GET':
        return res.status(200).json(localGetLabTests(doctorId as string));
      case 'POST': {
        const { name, normalValue, unit, group } = req.body;
        return res.status(201).json(localCreateLabTest(doctorId as string, { name, normalValue: normalValue || '', unit: unit || '', group: group || '' }));
      }
      case 'PUT': {
        const { id, name, normalValue, unit, group } = req.body;
        const updated = localUpdateLabTest(doctorId as string, Number(id), { name, normalValue: normalValue || '', unit: unit || '', group: group || '' });
        if (!updated) return res.status(404).json({ error: 'Not found' });
        return res.status(200).json(updated);
      }
      case 'DELETE': {
        const { id } = req.body;
        localDeleteLabTest(doctorId as string, Number(id));
