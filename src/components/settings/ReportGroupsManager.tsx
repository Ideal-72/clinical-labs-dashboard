...
interface ReportGroup {
    id: number;
    name: string;
    testGroups: string; // Comma separated string
}

export default function ReportGroupsManager() {
    ...
    const [filterTestGroup, setFilterTestGroup] = useState('');

    // Export dropdown
    const [exportDropdownOpen, setExportDropdownOpen] = useState(false);

    const [availableTestGroups, setAvailableTestGroups] = useState<string[]>([]);
    ...
    const fetchReportGroups = async () => {
        ...
        try {
            ...
            setReportGroups(Array.isArray(data) ? data : []);

            // Extract unique test groups from report groups for filter dropdown
            if (Array.isArray(data)) {
                const allTestGroups = new Set<string>();
                data.forEach(group => {
                    if (group.testGroups) {
                        group.testGroups.split(',').forEach((tg: string) => {
                            const trimmed = tg.trim();
                            if (trimmed) allTestGroups.add(trimmed);
                        });
                    }
                });
                setAvailableTestGroups([...allTestGroups].sort());
            }
        } catch (error) {
    ...
    // Sort, search and filter report groups
    const sortedAndFilteredReportGroups = useMemo(() => {
        let filtered = [...reportGroups].sort((a, b) => a.name.localeCompare(b.name));

        // Apply search filter
        if (searchTerm) {
            filtered = filtered.filter(group =>
                group.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                group.testGroups.toLowerCase().includes(searchTerm.toLowerCase())
            );
        }
        ...
        if (filterTestGroup) {
            filtered = filtered.filter(group =>
                group.testGroups.split(',').map(t => t.trim()).includes(filterTestGroup)
            );
        }

        return filtered;
    }, [reportGroups, searchTerm, filterTestGroup]);


    // Pagination logic with filtered data
    const totalPages = useMemo(() => {
        return pageSize === 0 ? 1 : Math.ceil(sortedAndFilteredReportGroups.length / pageSize);
    }, [sortedAndFilteredReportGroups.length, pageSize]);

    const paginatedData = useMemo(() => {
        if (pageSize === 0) return sortedAndFilteredReportGroups; // Show all
        const startIndex = (currentPage - 1) * pageSize;
        return sortedAndFilteredReportGroups.slice(startIndex, startIndex + pageSize);
    }, [sortedAndFilteredReportGroups, currentPage, pageSize]);
    ...
    // Calculate dropdown position based on row position (same as others)
    ...
    const getPaginationText = () => {
        if (pageSize === 0) {
            return `Showing 1 to ${sortedAndFilteredReportGroups.length} of ${sortedAndFilteredReportGroups.length} entries`;
        }
        const startIndex = (currentPage - 1) * pageSize + 1;
        const endIndex = Math.min(currentPage * pageSize, sortedAndFilteredReportGroups.length);
        return `Showing ${startIndex} to ${endIndex} of ${sortedAndFilteredReportGroups.length} entries`;
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            alert('Error saving report group');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async (id: number) => {
        if (confirm('Are you sure you want to delete this report group?')) {
            try {
                const response = await fetch('/api/report-groups', {
                    method: 'DELETE',
                    headers: {
                ...
                alert('Error deleting report group');
            }
        }
        setDropdownOpen(null);
    };
    ...
    const handleExport = async (format: string) => {
        const data = sortedAndFilteredReportGroups;
        const headers = ['Sl.No', 'Report Group Name', 'Included Test Groups'];
    ...
    const exportToCSV = (data: ReportGroup[], headers: string[]) => {
        ...
        link.href = url;
        link.download = `report-groups-${dateTime.filename}.csv`;
        link.click();
        URL.revokeObjectURL(url);
    };

    const exportToExcel = async (data: ReportGroup[], headers: string[]) => {
        const dateTime = getISTDateTime();

        const workbook = new ExcelJS.Workbook();
        const worksheet = workbook.addWorksheet('Report Groups');
        ...
        worksheet.columns.forEach(column => {
            column.width = 30; // Wider columns for this report
        });
        ...
        const link = document.createElement('a');
        link.href = url;
        link.download = `report-groups-${dateTime.filename}.xlsx`;
        link.click();
        URL.revokeObjectURL(url);
    };

    const exportToPDF = (data: ReportGroup[], headers: string[]) => {
        const dateTime = getISTDateTime();
        const doc = new jsPDF();
        ...
        doc.text('Report Groups Report', 20, 25);
        ...
        autoTable(doc, {
            ...
            columnStyles: {
                ...
                1: { cellWidth: 50 }, // Name
                2: { cellWidth: 110 }, // Test Groups  
            }
        });

        doc.save(`report-groups-${dateTime.filename}.pdf`);
    };
        ...
        <div className="space-y-6">
            ...
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-muted-foreground">Manage report categories and included test groups</p>
                </div>
            ...
            <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4 bg-card p-4 rounded-lg border border-border shadow-sm">
                <div className="flex flex-col sm:flex-row gap-4 flex-1">
                    <div className="relative flex-1">
                        ...
                        <input
                            type="text"
                            placeholder="Search by name or included test groups..."
                            value={searchTerm}
                            onChange={(e) => {
                    ...
                    <div className="sm:w-56">
                        <select
                            ...
                            onChange={(e) => {
                                setFilterTestGroup(e.target.value);
                                setCurrentPage(1);
                            }}
                        ...
                        >
                            <option value="">Contains Test Group</option>
                ...
                <div className="text-sm text-muted-foreground whitespace-nowrap">
                    {searchTerm || filterTestGroup ? `Found ${sortedAndFilteredReportGroups.length} results` : `${reportGroups.length} total entries`}
                </div>
            </div>
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-4 text-sm text-foreground">
                                                <div className="flex flex-wrap gap-1">
                                                    {group.testGroups.split(',').slice(0, 3).map((tag, i) => (
                                                    ...
                                                    {group.testGroups.split(',').length > 3 && (
                                                        <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
                                                            +{group.testGroups.split(',').length - 3} more
                                                        </span>
                                                    )}
                                                </div>
                                            </td>
                        ...
                        >
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <textarea
                                        value={formData.testGroups}
                                        onChange={(e) => setFormData({ ...formData, testGroups: e.target.value })}
                                        required
                                        rows={3}
